...
export default function PlaceDetailPage({
  ...
  useEffect(() => {
    ...
    const fetchPlaceDetails = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/places/${placeId}`); // Usamos a variável 'placeId'
  ...
  if (error || !place) {
    return (
      <div className="text-red-400 min-h-screen flex items-center justify-center">
        {error}
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-white p-4 sm:p-6 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gray-800 rounded-lg p-6 space-y-6 mb-8">
        ...
        {place.reviews && place.reviews.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold mb-6">
              Comentários do Google ({place.reviews.length})
            </h2>
            <div className="space-y-6">
              {place.reviews.slice(0, 5).map(
                (
                  review,
                  index // Mostra até 5 reviews
                ) => (
                  <div
                    key={index}
